...
int main( int argc, char **argv )
{
	...
	for ( k = 0; k < 3; k ++)
	{
        ...
        ************Calculate the Area***********/
        ...
        xbar=xbar/A;
        ybar=ybar/A;
        /********generate cross at the center****/
        bimage[(int)ybar][(int)xbar]=128;

        for(i=1;i<10;i++)
        {
            bimage[(int)ybar-i][(int)xbar]=128;
            bimage[(int)ybar+i][(int)xbar]=128;
            bimage[(int)ybar][(int)xbar+i]=128;
            bimage[(int)ybar][(int)xbar-i]=128;
        }
        //translate to bottom left corner coordinate
        ybar=ROWS-ybar;
